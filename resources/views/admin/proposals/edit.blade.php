    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.proposals.update", [$proposal->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job') ? 'is-invalid' : '' }}" name="job_id" id="job_id" required>
                    @foreach($jobs as $id => $job)
                        <option value="{{ $id }}" {{ ($proposal->job ? $proposal->job->id : old('job_id')) == $id ? 'selected' : '' }}>{{ $job }}</option>
                    @endforeach
                </select>
                @if($errors->has('job'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('freelancer') ? 'is-invalid' : '' }}" name="freelancer_id" id="freelancer_id" required>
                    @foreach($freelancers as $id => $freelancer)
                        <option value="{{ $id }}" {{ ($proposal->freelancer ? $proposal->freelancer->id : old('freelancer_id')) == $id ? 'selected' : '' }}>{{ $freelancer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('proposal_text') ? 'is-invalid' : '' }}" name="proposal_text" id="proposal_text" required>{{ old('proposal_text', $proposal->proposal_text) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('budget') ? 'is-invalid' : '' }}" type="text" name="budget" id="budget" value="{{ old('budget', $proposal->budget) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('delivery_time') ? 'is-invalid' : '' }}" type="text" name="delivery_time" id="delivery_time" value="{{ old('delivery_time', $proposal->delivery_time) }}" required>
